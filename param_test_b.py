"""
参数测试 B — 马丁格尔变体
参数：冷却=3天，上涨底仓=15000，加仓触发=跌>5%，反弹卖出=2×ATR，加仓金额=[20000,10000,5000]

逻辑：
- MA20<MA60=下跌环境→马丁格尔加仓
- MA20>=MA60=上涨环境→持有不动
- 初始现金=100000-15000=85000，position=15000
- 跌>5%时加仓（冷却3天），反弹2×ATR或RSI>70卖出加仓部分
- 最多加仓3次
"""
import pandas as pd
import numpy as np
...
def martingale_param_b(df, init_cash=100000, base_pos=15000,
...
print("=" * 90)
print("参数测试 B — 马丁格尔变体")
print("参数：冷却=3天 | 上涨底仓=15000 | 加仓触发=跌>5% | 反弹卖出=2×ATR")
...
if results:
    ...
    print(f"\n{'=' * 90}")
    print(f"总结：4个市场中{win_count}个跑赢基准 | 平均Alpha={avg_alpha:+.2%} | 平均回撤={avg_dd:.2%} | 平均交易={avg_trades:.0f}次")
    print(f"{'=' * 90}")
