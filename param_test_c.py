"""
参数测试 C — 马丁格尔自适应变体
参数：冷却=5天，上涨底仓=15000，加仓触发=跌>3%，反弹卖出=1.5×ATR，加仓金额=[20000,10000,5000]

逻辑：
- MA20<MA60=下跌环境→马丁格尔加仓
- MA20>=MA60=上涨环境→持有不动
- 初始现金=100000-15000=85000，position=15000
- 跌>3%时加仓（冷却5天），反弹1.5×ATR或RSI>70卖出加仓部分
- 最多加仓3次
"""
import pandas as pd
import numpy as np
...
def martingale_param_c(df, init_cash=100000, base_pos=15000,
                       max_add=3, add_amounts=[20000, 10000, 5000],
                       cooldown=5, dip_threshold=-0.03, atr_multiplier=1.5):
    """
    马丁格尔自适应策略 - 参数C版本
    """
...
print("=" * 90)
print("参数测试 C — 马丁格尔自适应变体")
print("参数: 冷却=5天 | 上涨底仓=15000 | 加仓触发=跌>3% | 反弹卖出=1.5×ATR")
print("加仓金额: [20000, 10000, 5000] | 初始现金=85000 | 总仓位=100000")
print("=" * 90)
...
if alphas:
    ...
    print(f"\n{'=' * 90}")
    print(f"总结: 4个市场平均Alpha={avg_alpha:+.2%}, {positive}/4个市场跑赢基准")
    print(f"参数C(冷却5天+跌3%加仓+1.5xATR卖出)表现: {'优秀' if avg_alpha > 0.05 else '良好' if avg_alpha > 0 else '一般'}")
    print(f"{'=' * 90}")
