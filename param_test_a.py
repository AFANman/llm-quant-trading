"""
参数测试 A — 马丁格尔自适应回测
参数：冷却=0天，上涨底仓=20000，加仓触发=跌>5%，反弹卖出=2×ATR，加仓金额=[20000,10000,5000]

逻辑：
- MA20<MA60=下跌环境→马丁格尔加仓
- MA20>=MA60=上涨环境→持有不动
- 初始现金=100000-上涨底仓，position=上涨底仓
- 跌>5%时加仓（冷却0天），反弹2×ATR或RSI>70卖出加仓部分
- 最多加仓3次
"""
import pandas as pd
import numpy as np
...
COOLDOWN = 0           # 冷却天数
BASE_POS = 20000       # 上涨底仓
DIP_PCT = -0.05        # 加仓触发：跌>5%
MAX_ADD = 3            # 最多加仓3次
ADD_AMOUNTS = [20000, 10000, 5000]  # 加仓金额（递减）
INIT_CASH = TOTAL_CAPITAL - BASE_POS  # 80000


def martingale_param_test(df, init_cash=INIT_CASH, base_pos=BASE_POS,
                          max_add=MAX_ADD, add_amounts=ADD_AMOUNTS,
                          cooldown=COOLDOWN, dip_pct=DIP_PCT):
    """马丁格尔自适应回测"""
...
print("=" * 95)
print("参数测试 A — 马丁格尔自适应回测")
print(f"参数: 冷却={COOLDOWN}天 | 底仓={BASE_POS} | 加仓触发=跌>{abs(DIP_PCT)*100:.0f}% | "
      f"反弹卖出=2×ATR | 加仓金额={ADD_AMOUNTS}")
print(f"初始: 现金={INIT_CASH} + 持仓={BASE_POS} = {TOTAL_CAPITAL}")
print("=" * 95)
...
for code, name, s, e in tests:
    ...
    try:
        ...
        r = martingale_param_test(df)
        results.append({'code': code, 'name': name, **r})
...
if alphas:
    ...
    print(f"\n{'=' * 95}")
    print(f"总结: 平均Alpha={avg_alpha:+.2%} | {win_count}/{len(alphas)}个市场跑赢基准 | "
          f"冷却{COOLDOWN}天+底仓{BASE_POS}+加仓{ADD_AMOUNTS}")
    print(f"{'=' * 95}")
